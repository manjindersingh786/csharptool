...
void _31115_SmsppImplementation(IN_ARG puMonoByte pumbSmsData, IN_ARG uMonoByte umbLenSmsData) {
	...
	/*Storing bytes of SMS-TPDU value part upto UDL*/
	_7816_4_readUpdateFile(fhaFile, pumbSmsData, OFFSET_SECURED_MESSAGE, LEN_SMS_TPDU_TO_UDL, ACC_MODE_FILE_UPDATE);
	...
	if (POR_OK == umbRSC) {
		...
		if (0x01 != _102226_ProcessRfm(pumbSmsData[umbIndex + 1])) {
...
void _31115_preparePOR(IN_ARG uMonoByte umbRSC)
{
	...
	_7816_4_readUpdateFile(fhaFile, udbsdbuf, FILE_OFFSET_SPI1, 0x04, ACC_MODE_FILE_READ);

	/*if SMS SUBMIT*/
	if (udbsdbuf[0x01] & MASK_SMS_SUBMIT) {
		...
		_7816_4_readUpdateFile(fhaFile, PROACTIVE_BUFF + OFFSET_TP_DA, FILE_OFFSET_TP_OA_LEN, (0x02 + TP_OA_MAX_LEN), ACC_MODE_FILE_READ);
	...
	//include TAR 
	_7816_4_readUpdateFile(fhaFile, PROACTIVE_BUFF + umbIndex, FILE_OFFSET_TAR, 0x03, ACC_MODE_FILE_READ);

	umbIndex = umbIndex + 0x03; //mpw index at counter 
	...
	if ((CNTR_LOW == umbRSC)
		...
		//include CNTR
		_7816_4_readUpdateFile(fhaFile, PROACTIVE_BUFF + umbIndex, FILE_OFFSET_CNTR, LENGTH_CNTR, ACC_MODE_FILE_READ);

	}
	else {//set CNTR value 00
		mem_set(PROACTIVE_BUFF + umbIndex, 0x00, LENGTH_CNTR);
	}

	umbIndex = umbIndex + LENGTH_CNTR;
	...
	_7816_4_readUpdateFile(fhaFile, udbsdbuf + 4, OFFSET_RESPONSE_MESSAGE, 0x03, ACC_MODE_FILE_READ);

	udbResSize = mm_makeUdiByte(udbsdbuf + 0x05, 0x00);

	DBG_FUNC("Store Secure Data Length  = %2\n", udbResSize);
	...
	_7816_4_readUpdateFile(fhaFile, PROACTIVE_BUFF + umbIndex, OFFSET_RESPONSE_MESSAGE + udbsdbuf[0x04], udbResSize, ACC_MODE_FILE_READ);
	//adding udbsdbuf for vacant bytes
	...
	if (0x00 != umbChecksumLen) {//if RC/CC/DS available
		...
		if (POR_REDUNDANCY_CHECK == (udbsdbuf[0x01] & MASK_SPI2_CHECKSUM)) {
			...
			algo_chksm_finalize(&cChecksumObj, (PROACTIVE_BUFF + indexUDHL + 0x10 + umbChecksumLen), (udbResSize + umbPadding), umbChecksum);
		}
		else {
	...
	if (udbsdbuf[0x01] & 0x20) {
		...
		if (0x7F < PROACTIVE_BUFF[OFFSET_SMS_TPDU_LEN]) {
			...
			PROACTIVE_BUFF[OFFSET_PROACT_LEN] = 0x81;
