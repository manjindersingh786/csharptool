...
//	Write BYTE.
//	Return value:	0x01 - success,0x02 - operation error,0x04 - verification failed
BYTE	Write_Bytes(BYTE * pDest,const BYTE * pSrc,HALFW len)
{
	HALFW	i;
	...
	if(mem_cmp((BYTE *)pDest,pSrc,len))
	{
...
BYTE	Write_page(WORD FlashAddr,BYTE * pSrc)
{
	...
	{
		if(mem_cmp((BYTE *)(FlashAddr + i * RowSize),pSrc + i * RowSize,RowSize))
		{
			if((res = Write_ROW_Bytes((BYTE *)(FlashAddr + i * RowSize),pSrc + i * RowSize,RowSize)) != 0x01)
			{
				return res;
			}
		}
