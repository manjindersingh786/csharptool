...
void	ReadFlash(HALFW foffset,BYTE * RAMbuf,HALFW length)
{
	...
	mem_cpy(RAMbuf,(BYTE *)FlashAddr,length);
}
...
void	UpdateMemory(HALFW foffset,BYTE * RAMbuf,BYTE length)
{
	...
	{   					
		...
		for(intRemain = length; intRemain; intRemain  -= toDo)
		{
			...
			mem_cpy(FlashBuffer,(BYTE *)FlashAddr,PageSize);
			
	
			mem_cpy(&FlashBuffer[poffset],&RAMbuf[counter],toDo);
			counter += toDo;			
...
void	ReturnToBL(void)
{
	...
	Foffset = 0x0000;
	Erase_Page(Foffset);
	Write_Bytes((BYTE *)Foffset,VectorTable,0x78);
